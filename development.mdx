...
Follow these steps to install and run Ezaalah on your operating system.
...
By default, Ezaalah uses port 3000. You can customize the port Ezaalah runs on by using the `--port` flag. For example, to run Ezaalah on port 3333, use this command:

```bash
mint dev --port 3333
```

If you attempt to run Ezaalah on a port that's already in use, it will use the next available port:
...
Please note that each CLI release is associated with a specific version of Ezaalah. If your local preview does not align with the production version, please update the CLI:
